...
table1_vars<-c("Age","Sex","BMI","BMI_cat","DM","HTN","COPD","CoronaryArteryDisease","ChronicRenalDisease",
               ...
               "PrimaryRecurrent","TumorSize","TumorHistologicSubtype","FNCLCC",
               "Resection","Resection_Colon","Resection_SmallBowel",
               "Resection_Pancreas","Resection_Liver","Resection_MajorVesselResection",
...
# print(table1_Group, showAllLevels = TRUE, exact = table1_factor_vars[table1_factor_vars %in% vars.fisher])
# 
# #Group 1 vs 23
...
# print(table1_Group1vs23, showAllLevels = TRUE, exact = table1_factor_vars[table1_factor_vars %in% vars.fisher1vs23])
# 
# #Group 1 vs 2
...
# print(table1_Group1vs2, showAllLevels = TRUE, exact = vars.fisher1vs2)

#Total table

table1_total<- lapply(table1_vars, function(va){
  if (va %in% table1_conti_vars){
    ...
    p1vs23 <- t.test(as.formula(paste0(va, "~ Group1vs23")), data = out, var.equal = F)$p.value
    p1vs2 <- t.test(as.formula(paste0(va, "~ Group1vs2")), data = out, var.equal = F)$p.value
    ...
    p1vs23 <- ifelse(va %in% vars.fisher1vs23,
                     fisher.test(table(out[[va]],out[["Group1vs23"]]))$p.value,
                     chisq.test(table(out[[va]],out[["Group1vs23"]]))$p.value)
    p1vs2 <- ifelse(va %in% vars.fisher1vs2,
                     fisher.test(table(out[[va]],out[["Group1vs2"]]))$p.value,
                     chisq.test(table(out[[va]],out[["Group1vs2"]]))$p.value)
    
    out.final <- cbind(c(paste0(va, " (%)"), rep("", nrow(tb.out) - 1)),
                       rownames(tb),
                       tb.out,
                       c(ifelse(p < 0.001, "< 0.001", round(p, 3)), rep("", nrow(tb.out) - 1)),
                       c(ifelse(p1vs23 < 0.001, "< 0.001", round(p1vs23, 3)), rep("", nrow(tb.out) - 1)),
                       c(ifelse(p1vs2 < 0.001, "< 0.001", round(p1vs2, 3)), rep("", nrow(tb.out) - 1)))
    return(out.final)
  }
}) %>% Reduce(rbind, .)

table1_total<-rbind(c("n","",as.integer(summary(out$Group))[1:3],"","",""),table1_total)
colnames(table1_total) <- c("Variable", "Subgroup", "Group1", "Group2","Group3", "p-value(all)","p-value(1vs23)","p-value(1vs2)")
rownames(table1_total)<-NULL
datatable(table1_total)
...
# print(table2_Group, showAllLevels = TRUE, exact = table2_factor_vars[table2_factor_vars %in% vars.fisher])
# 
# #Group 1 vs 23
...
# print(table2_Group1vs23, showAllLevels = TRUE, exact = table2_factor_vars[table2_factor_vars %in% vars.fisher1vs23])
# 
# #Group 1 vs 2
...
# print(table2_Group1vs2, showAllLevels = TRUE, exact = table2_factor_vars[table2_factor_vars %in% vars.fisher1vs2])

#Total table
table2_total<- lapply(table2_vars, function(va){
...
table2_total<-rbind(c("n","",as.integer(summary(out$Group))[1:3],"","",""),table2_total)
colnames(table2_total) <- c("Variable", "Subgroup", "Group1", "Group2","Group3", "p-value(all)","p-value(1vs23)","p-value(1vs2)")
datatable(table2_total)
